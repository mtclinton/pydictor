...
def main():
    parser = argparse.ArgumentParser(
        prog="PyDictor", description="Show the weather in the terminal"
    )
    parser.add_argument("address", help="Address to check the weather")
    args = parser.parse_args()
    l = "en-US"
    x = requests.get(
        f"https://nominatim.openstreetmap.org/search?q={args.address}&accept-language={l}&limit=1&format=jsonv2"
    )
    location = x.json()[0]
